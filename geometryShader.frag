...
void main()
{
	if (selected == 1) 
	{
		color = vec4(positionColor.x * 1.2,positionColor.y * 0.8,positionColor.z * 0.8, 1.0f);
		return;
	}
	...
	vec4 amb = vec4(0.3f,0.3f,0.3f,1.0f);
	vec4 dif = vec4(0.6f,0.6f,0.6f,1.0f);
	vec4 spec = vec4(0.4f,0.4f,0.4f,1.0f);
